...
<% }else{ %>
	...
	<div class="login-btn-group">
		...
		<% if(config.design.login_style == 'popup' && parseInt(model.config.option.login.display)){ %> 
			<!-- <div class="btn-group" role="group">	
				<button id="login_button_popup" type="button" class="btn btn-default" onclick="window.location = 'index.php?route=account/login&r=checkout'"><%= model.config.option.login.title %></button>
			</div> -->
		<% } %>
		</div>
	</div>
