...
class ModelShippingJoseanMatiasFaixaCEP extends Model {
    ...
    function getQuote($address) {
       ...
       //  $x = $this->db->query('SELECT * FROM oc_setting WHERE `key` = "free_status"');
       //  $y = $this->db->query('SELECT * FROM oc_setting WHERE `key` = "free_total"');
        

       // $freeShippingStatus = $x->row['value'];
       // $freeShippingTitle = $y->row['title'];
        ...
        if ($status && isset($faixa_cep_cost)) {
            ...
            //     $quote_data['joseanmatias_faixa_cepGratis'] = array(

            //         'code' => 'joseanmatias_faixa_cep.joseanmatias_faixa_cepGratis',
