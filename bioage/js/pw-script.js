jQuery(function(){
    ...
    if(jQuery('body.catalog-product-view').size() > 0){
        ...
        jQuery('body.catalog-product-view .product-shop .preco-parcelado-sem-juros .upto').prepend('No Cartão: ');
    ...
    if(jQuery('body.cms-onde-encontrar').size() > 0){
      ...
      jQuery('.det').hide();
          ...
          jQuery('#modalDistri input:nth-child(6)').prop('placeholder','Região de interesse (cidade/estado)');
  ...
  if(jQuery('.checkout-onepage-index').size() > 0){
    jQuery('#billing-progress-opcheckout dt').text('Endereço de Entrega');
    jQuery('#opc-billing .step-title h2 span').text('Informações e Endereço');
  }
